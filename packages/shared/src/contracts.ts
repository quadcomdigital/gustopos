...
export const printBridgeOnboardingSecretCreateResponseSchema = z.object({
  // Literal (not the enum) so the response is narrowly typed as "long" — this lets
  // `created.mode === "code-6digit"` properly exclude the long variant in TS narrowing.
...
// 6-digit ephemeral pairing code (90s TTL). Used by the wizard UI: admin generates it,
// shows QR + numeric to the kitchen PC, kitchen PC presents it in X-Print-Bridge-Key
// and is bound to the underlying onboarding secret on first use.
...
// ─── Local browser-bridge config (Phase E) ────────────────────────────────
...
// ─── Print-bridge response envelopes (Phase E) ────────────────────────────────
...
// PrintArea type alias (consumed by inventory modals + bridge panel)
