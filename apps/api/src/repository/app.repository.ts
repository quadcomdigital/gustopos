...
export class AppRepository {
  ...
  async testPrintFromBridge(bridgeId: string, area: PrintArea, message?: string): Promise<{ jobId: string; bridgeId: string; area: PrintArea; orderId: string }> {
    ...
    constructor(@Inject(StaffRepository) private readonly staffRepo: StaffRepository, @Inject(ShiftsRepository) private readonly shiftsRepo: ShiftsRepository, @Inject(SuppliersRepository) private readonly suppliersRepo: SuppliersRepository, @Inject(InventoryRepository) private readonly inventoryRepo: InventoryRepository, @Inject(TablesRepository) private readonly tablesRepo: TablesRepository, @Inject(ConsumerRepository) private readonly consumerRepo: ConsumerRepository, @Inject(CustomerRepository) private readonly customerRepo: CustomerRepository, @Inject(PaymentsRepository) private readonly paymentsRepo: PaymentsRepository) {
    }
}
