...
export const inventory = pgTable(
  "inventory",
  {
    ...
    isActive: integer("is_active").notNull().default(1),
    isStockTracked: integer("is_stock_tracked").notNull().default(1),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [
...
export const printBridgeOnboardingSecrets = pgTable("print_bridge_onboarding_secrets", {
  ...
  shortCodeExpiresAt: timestamp("short_code_expires_at", { withTimezone: true }),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
  ...
  // Partial index lets verifyBridgeOrOnboardingSecret lookup 6-digit codes without scanning null rows.
  uniqueIndex("print_bridge_onboarding_secrets_short_code_idx")
    .on(t.shortCodeHash)
